...
int gen_instr_valid()
{
    return ((if_id_next->icode) == (I_NOP)||(if_id_next->icode) == (I_HALT)||(if_id_next->icode) == (I_RRMOVL)||(if_id_next->icode) == (I_IRMOVL)||(if_id_next->icode) == (I_RMMOVL)||(if_id_next->icode) == (I_MRMOVL)||(if_id_next->icode) == (I_ALU)||(if_id_next->icode) == (I_ALUI)||(if_id_next->icode) == (I_JXX)||(if_id_next->icode) == (I_CALL)||(if_id_next->icode) == (I_RET)||(if_id_next->icode) == (I_PUSHL)||(if_id_next->icode) == (I_POPL)||(if_id_next->icode) == (I_LSM));
}
...
int gen_new_E_srcA()
{
    return (((if_id_curr->icode) == (I_RRMOVL)||(if_id_curr->icode) == (I_RMMOVL)||(if_id_curr->icode) == (I_ALU)||(if_id_curr->icode) == (I_PUSHL)) ? (if_id_curr->ra) : ((if_id_curr->icode) == (I_POPL)||(if_id_curr->icode) == (I_RET)) ? (REG_ESP) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 0)) ? (REG_ESI) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 1)) ? (REG_EAX) : 1 ? (REG_NONE) : 0);
}

int gen_new_E_srcB()
{
    return (((if_id_curr->icode) == (I_ALU)||(if_id_curr->icode) == (I_ALUI)||(if_id_curr->icode) == (I_RMMOVL)||(if_id_curr->icode) == (I_MRMOVL)) ? (if_id_curr->rb) : ((if_id_curr->icode) == (I_PUSHL)||(if_id_curr->icode) == (I_POPL)||(if_id_curr->icode) == (I_CALL)||(if_id_curr->icode) == (I_RET)) ? (REG_ESP) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 0)) ? (REG_ESI) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 1)) ? (REG_EDI) : 1 ? (REG_NONE) : 0);
}

int gen_new_E_dstE()
{
    return (((if_id_curr->icode) == (I_RRMOVL)||(if_id_curr->icode) == (I_IRMOVL)||(if_id_curr->icode) == (I_ALU)||(if_id_curr->icode) == (I_ALUI)) ? (if_id_curr->rb) : ((if_id_curr->icode) == (I_PUSHL)||(if_id_curr->icode) == (I_POPL)||(if_id_curr->icode) == (I_CALL)||(if_id_curr->icode) == (I_RET)) ? (REG_ESP) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 0)) ? (REG_ESI) : 1 ? (DEST_NONE) : 0);
}

int gen_new_E_dstM()
{
    return (((if_id_curr->icode) == (I_MRMOVL)||(if_id_curr->icode) == (I_POPL)) ? (if_id_curr->ra) : (((if_id_curr->icode) == (I_LSM)) & ((if_id_curr->ifun) == 0)) ? (REG_EAX) : 1 ? (DEST_NONE) : 0);
}
...
int gen_aluB()
{
    return (((id_ex_curr->icode) == (I_RMMOVL)||(id_ex_curr->icode) == (I_MRMOVL)||(id_ex_curr->icode) == (I_ALU)||(id_ex_curr->icode) == (I_ALUI)||(id_ex_curr->icode) == (I_CALL)||(id_ex_curr->icode) == (I_PUSHL)||(id_ex_curr->icode) == (I_RET)||(id_ex_curr->icode) == (I_POPL)||(id_ex_curr->icode) == (I_LSM)) ? (id_ex_curr->valb) : ((id_ex_curr->icode) == (I_RRMOVL)||(id_ex_curr->icode) == (I_IRMOVL)) ? 0 : 0);
}
...
int gen_mem_addr()
{
    return (((ex_mem_curr->icode) == (I_RMMOVL)||(ex_mem_curr->icode) == (I_PUSHL)||(ex_mem_curr->icode) == (I_CALL)||(ex_mem_curr->icode) == (I_MRMOVL)) ? (ex_mem_curr->vale) : ((ex_mem_curr->icode) == (I_POPL)||(ex_mem_curr->icode) == (I_RET)) ? (ex_mem_curr->vala) : (((ex_mem_curr->icode) == (I_LSM)) & ((ex_mem_curr->ifun) == 0)) ? (ex_mem_curr->vala) : 0);
}

int gen_mem_read()
{
    return (((ex_mem_curr->icode) == (I_MRMOVL)||(ex_mem_curr->icode) == (I_POPL)||(ex_mem_curr->icode) == (I_RET)) | (((ex_mem_curr->icode) == (I_LSM)) & ((ex_mem_curr->ifun) == 0)));
}
