...
word_t wb_destE = DEST_NONE;
word_t wb_valE = 0;
word_t wb_destM = DEST_NONE;
word_t wb_valM = 0;
word_t mem_addr = 0;
word_t mem_data = 0;
...
static exc_t sim_step_pipe(int max_instr)
{
    ...
    /* Performance monitoring */
    if (mem_wb_curr->exception != EXC_BUBBLE) {
	starting_up = 0;
	instructions++;
	cycles++;
    } else {
...
void signal_register_update(reg_id_t r, word_t val) {
    int code;
    sprintf(tcl_msg, "setReg %d %u 1", (int) r, (unsigned) val);
...
void create_memory_display() {
	...
	for (i = 0; i < memCnt && code == TCL_OK; i+=4) {
	    ...
	    sprintf(tcl_msg, "setMem %d %u", addr, (unsigned) val);
	    code = Tcl_Eval(sim_interp, tcl_msg);
	}
	if (code != TCL_OK) {
...
id_ex_ele bubble_id_ex = { I_NOP, 0, 0, 0, 0,
			   REG_NONE, REG_NONE, DEST_NONE, DEST_NONE,
			   EXC_BUBBLE, 0};

ex_mem_ele bubble_ex_mem = { I_NOP, 0, FALSE, 0, 0,
			     DEST_NONE, DEST_NONE, EXC_BUBBLE, 0};

mem_wb_ele bubble_mem_wb = { I_NOP, 0, 0, 0, DEST_NONE, DEST_NONE,
			     EXC_BUBBLE, 0};
...
int gen_F_bubble();		/* JB déclaration déplacée */
...
void do_if_stage()
{
    ...
    /* JB */
    if (gen_F_bubble()) {
    	memcpy (if_id_next, &bubble_if_id, if_id_state->count);
    	// fprintf (stderr, "%s\n", format_if_id(if_id_next));
    	return;
    }
...
void do_ex_stage()
{
    ...
    ex_mem_next->takebranch = id_ex_curr->icode == I_JXX &&
	e_bcond;

    if (id_ex_curr->icode == I_JXX)
